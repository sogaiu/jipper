...
(defn is-right-directive?
  [a-node]
  (def [node-type _ node-value] a-node)
  (when (not= :comment node-type)
    (break false))
  #
  (def m (peg/match ~(sequence "#<" :s+
                               (capture (some 1))
                               :s* -1)
                    node-value))
  (when (not m)
...
(comment
  ...
  (is-right-directive? [:comment @{} "#< :jell/no-rename"])
  # =>
  @{:jell/no-rename true}

  (is-right-directive? [:comment @{} "#< {:jell/no-rename true}"])
  # =>
  {:jell/no-rename true}
  ...
  (is-right-directive? [:symbol @{} "jell/no-rename"])
  # =>
  false
  ...
  (while (not (j/end? cur-zloc))
    (when-let [found (match (j/node cur-zloc) [:symbol _ name]
                       (when (get sym-tbl name)
                         name))]
      (def no-rename
        (when-let [peek-zloc (j/right-skip-ws cur-zloc)
                   dd (is-right-directive? (j/node peek-zloc))]
          (get dd :jell/no-rename)))
      #
      (when (not no-rename)
